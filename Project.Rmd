...
Then we changed categorical variables to integers which can be then furhter used in ml.
...
```
As dataset is imbalanced, with 88% of observations representing yes value and only 12% representing no value, we balanced out datast using majority class technique. After applying it, we see that number of observation is equal for both outcomes.
```{r}

bankcl_no <- bankcl %>% filter(deposit_cat == 0)
bankcl_yes <- bankcl %>% filter(deposit_cat == 1)

bankcl_yes_over <- bankcl_yes %>% sample_n(nrow(bankcl_no), replace = TRUE)

bankcl_balanced <- bind_rows(bankcl_no, bankcl_yes_over)

table(bankcl_balanced$deposit_cat)
```

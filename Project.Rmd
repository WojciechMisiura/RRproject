...
Dataset will be loaded from github using 'remote' library to ensure that users do not have to change the path to their local path. Originally, data was loaded from local path which creates problem when one want to re-run project in own environment.

```{r}
url<-"https://raw.githubusercontent.com/WojciechMisiura/RRproject/main/bank.csv" 
url<-"/Users/wojciech/project/studia/rr_2024/project/RRproject/bank.csv"
...
#
average of duration within each age group
...
bank_data_filtered <- bank_data %>% 
  filter(deposit == "yes") # Filter data where deposit is "yes"
...
As some jobs are grouped into groups such us blue-collar, we categorizes the job types in the 'bank_data' dataset into groups such us: 'white-collar', 'pink-collar', and 'other'.
...
In dataset there are 4 types of answears - unknown, other, failure and success. As there is no difference between unknown and other for the model, we have a code to overwrite all other values to unknown.
...
We employed the GBM model because of its proficiency in handling various types of data features and its robustness in predicting binary outcomes
...
# Create a factor version of actual and predicted to use in confusion matrix
...
# Model accuracy from the confusion matrix
